...
sub _C_list {
  ...
  if ($this->{hideEmptyAttachmentDirs}) {
    foreach my $topic ($cat->getTopics) {
      $topic = '.' . $topic unless $this->_hasAttachments($info->{web}, $topic);
      push @list, $topic;
    }
  } else {
...
sub _C_stat {
  ...
  $stat[2] = ($stat[2] & ~(00222)) if $cat->{origWeb} ne $info->{web} || $info->{category} eq NOCAT;
...
sub _C_test {
  ...
  return ($cat ? $this->_haveAccess('VIEW', $cat->{origWeb}, $catName) : 0) if $type =~ /r/i;
  return ($cat ? ($cat->{origWeb} eq $info->{web}?1:0) : 0) if $type =~ /w/i;
